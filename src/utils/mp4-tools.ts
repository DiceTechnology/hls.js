...
function createFakeSinfBox(fourCC: string, entry: Uint8Array) {
  const encType =
    fourCC === 'avc1'
      ? [0x65, 0x6e, 0x63, 0x76] // 'encv'
...
export function fakeEncryption(initSegment: Uint8Array) {
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    switch (fourCC) {
      ...
      case 'mp4a':
        {
          const entryWithSinf = createFakeSinfBox(fourCC, entry);
          newEntries.push(entryWithSinf);
        }
        break;
      default:
        break;
