...
export function patchLevelDetailsForPlayReadyDrm(levelDetails: LevelDetails) {
  const firstFragment = levelDetails.fragments[0];
  const firstEncryptedFragment = levelDetails.encryptedFragments?.[0];
  if (!firstFragment.levelkeys && firstEncryptedFragment?.levelkeys) {
    const isDrmEncrypted =
      firstEncryptedFragment.levelkeys?.[KeySystemFormats.PLAYREADY];
    if (isDrmEncrypted) {
      logger.debug('Apply PlayReady workaround for level details');
      firstFragment.levelkeys = firstEncryptedFragment.levelkeys;
    }
  }
}
